...
for(my $i=0;$i<@prefiles;$i++) 
{
  ...
  if (@files>0)
  {
    foreach my $e(@files){
      ...
      my $com="mkdir -p $outd/pipe.rsem.$e/;$rsemCmd --bowtie-path $bowtiepath -p 8 $params_rsem --output-genome-bam --paired-end $str_files $rsemref $outd/pipe.rsem.$e/rsem.out.$e";  
  ...
  else
  {
	...
	#print "$outdir/pipe.tophat.$e/accepted_hits.bam\n";
        my $com="mkdir -p $outd/pipe.rsem.$e/;$rsemCmd --bowtie-path $bowtiepath -p 8 $params_rsem --output-genome-bam --calc-ci $str_files $rsemref $outd/pipe.rsem.$e/rsem.out.$e\n"; 
        print "$com\n";
