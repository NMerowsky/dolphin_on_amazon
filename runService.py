...
def main():
    ...
    hostname = str(os.popen("hostname").readlines())
    hostname = re.sub("[\[']+", "", hostname)
    part = hostname.split(".")
   
    command = python+" " + edir  + "/submitJobs.py -d "+ DBHOSTNAME + " -u "+ USERNAME + " -k "+ WKEY + " -o "+ OUTDIR + " -c " + bash_script_file + " -n " + SERVICENAME  + " -s " + SERVICENAME
    print command
    os.system(command)
